...
class DataManager: ObservableObject {
    ...
    // MARK: - Salary Carry Forward Operations
    
    func addSalaryPeriod(for employeeID: UUID, month: Date, totalSalaryDue: Double? = nil, notes: String? = nil) {
        guard let index = employees.firstIndex(where: { $0.id == employeeID }) else { return }
        
        // Get the actual salary due or use the employee's monthly salary
        let salaryDue = totalSalaryDue ?? employees[index].monthlySalary
        
        // Create a new unpaid salary period
        let unpaidPeriod = UnpaidSalaryPeriod(
            month: month,
            totalSalaryDue: salaryDue,
            paidAmount: 0.0,
            notes: notes
        )
        
        // Add to employee's unpaid periods
        employees[index].unpaidSalaryPeriods.append(unpaidPeriod)
        saveEmployees()
    }
    
    func updateSalaryPeriodPayment(for employeeID: UUID, periodID: UUID, paidAmount: Double) {
        guard let employeeIndex = employees.firstIndex(where: { $0.id == employeeID }),
              let periodIndex = employees[employeeIndex].unpaidSalaryPeriods.firstIndex(where: { $0.id == periodID }) else {
            return
        }
        
        // Get the period
        var period = employees[employeeIndex].unpaidSalaryPeriods[periodIndex]
        
        // Update the paid amount and remaining amount
        period.paidAmount += paidAmount
        period.remainingAmount = period.totalSalaryDue - period.paidAmount
        
        // Replace the period in the employee's array
        employees[employeeIndex].unpaidSalaryPeriods[periodIndex] = period
        saveEmployees()
    }
    
    func salaryPaymentWithCarryForward(
        amount: Double,
        date: Date,
        employeeID: UUID,
        paymentMethod: PaymentMethod = .bankTransfer,
        ...
        // 1. Get unpaid salary periods sorted by date (oldest first)
        let unpaidPeriods = employee.unpaidSalaryPeriods
            .filter { $0.remainingAmount > 0 }
            .sorted { $0.month < $1.month }
        
        var remainingPayment = amount
        var periodsUpdated: [UUID] = []
        
        // 2. Allocate payment to unpaid periods
        for period in unpaidPeriods {
            guard remainingPayment > 0 else { break }
            
            let paymentForPeriod = min(remainingPayment, period.remainingAmount)
            updateSalaryPeriodPayment(for: employeeID, periodID: period.id, paidAmount: paymentForPeriod)
            
            remainingPayment -= paymentForPeriod
            periodsUpdated.append(period.id)
        }
        
        // 3. Record the payment in employee's salary history
        let periodTexts = periodsUpdated.compactMap { periodID -> String? in
            guard let period = employee.unpaidSalaryPeriods.first(where: { $0.id == periodID }) else { return nil }
            let monthFormatter = DateFormatter()
            monthFormatter.dateFormat = "MMM yyyy"
            return monthFormatter.string(from: period.month)
        }
        
        let paymentNotes = notes ?? "Payment for periods: \(periodTexts.joined(separator: ", "))"
        
        // 4. Add the salary payment
        // Calculate rough start and end dates based on the periods covered
        let periodStart = unpaidPeriods.first?.month ?? Date().startOfMonth
        let periodEnd = unpaidPeriods.last?.month.endOfMonth ?? Date()
    ...
    // Helper function to automatically create salary periods for all employees
    func createMonthlySalaryPeriods(for month: Date) {
        for employee in employees {
            // Check if this period already exists
            let targetMonth = month.startOfMonth
            let periodExists = employee.unpaidSalaryPeriods.contains { period in
                let periodMonth = period.month.startOfMonth
                let targetMonthComponents = Calendar.current.dateComponents([.month, .year], from: targetMonth)
                let periodMonthComponents = Calendar.current.dateComponents([.month, .year], from: periodMonth)
                
                return periodMonthComponents.month == targetMonthComponents.month && 
                       periodMonthComponents.year == targetMonthComponents.year
            }
            
            if !periodExists {
                addSalaryPeriod(for: employee.id, month: targetMonth)
            }
        }
    }
    
    // Helper to get unpaid salary balance by month
    func getUnpaidSalaryByMonth(for employeeID: UUID) -> [(month: Date, amount: Double)] {
        guard let employee = employees.first(where: { $0.id == employeeID }) else { return [] }
        
        return employee.unpaidSalaryPeriods
            .filter { $0.remainingAmount > 0 }
            .map { ($0.month, $0.remainingAmount) }
            .sorted { $0.month < $1.month }
    }
} 