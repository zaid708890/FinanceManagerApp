...
struct Employee: Identifiable, Codable {
    ...
    var unpaidSalaryPeriods: [UnpaidSalaryPeriod] = []
    ...
    var totalUnpaidSalary: Double {
        return unpaidSalaryPeriods.reduce(0) { $0 + $1.remainingAmount }
    }
...
// Structure for tracking unpaid salary periods
struct UnpaidSalaryPeriod: Identifiable, Codable {
    var id = UUID()
    var month: Date  // First day of the month
    var year: Int
    var totalSalaryDue: Double
    var paidAmount: Double
    var remainingAmount: Double
    var notes: String?
    
    init(month: Date, totalSalaryDue: Double, paidAmount: Double, notes: String? = nil) {
        self.month = month.startOfMonth
        let calendar = Calendar.current
        self.year = calendar.component(.year, from: month)
        self.totalSalaryDue = totalSalaryDue
        self.paidAmount = paidAmount
        self.remainingAmount = totalSalaryDue - paidAmount
        self.notes = notes
    }
} 